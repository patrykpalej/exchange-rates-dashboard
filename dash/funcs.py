...
def draw_plot(df_orig, col, cols_list, second_label, time_range, plot_types):
    ...
    if 1 in plot_types:
        graph.add_trace(
            go.Scatter(x=df.index, y=df[col], marker=dict(color="blue"),
                       name=col.replace('-', ' / ')),
            secondary_y=False)

    if 2 in plot_types:
        graph.add_trace(
            go.Scatter(x=df.index, y=average_price,
