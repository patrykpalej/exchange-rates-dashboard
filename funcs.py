...
def draw_plot(df_orig, col, cols_list, second_label, time_range, plot_types):
    ...
    year_start = pd.to_datetime(df_orig['timestamp'].iloc[0]).year
    year_end = pd.to_datetime(df_orig['timestamp'].iloc[-1]).year
    ...
    if 1 in plot_types:
        graph.add_trace(
            go.Scatter(x=df['timestamp'], y=df[col], marker=dict(color="blue"),
                       name=col.replace('-', ' / ')), secondary_y=False)

    if 2 in plot_types:
        graph.add_trace(
            go.Scatter(x=df['timestamp'], y=average_price, marker=dict(color="red"),
                       name=second_label), secondary_y=True)

    # --- 4. Style plot
    graph.update_layout(margin=dict(t=25, r=0, l=0, b=35), showlegend=True)
    graph.update_xaxes(tickangle=45, tickfont=dict(size=10),
